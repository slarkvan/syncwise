...
Browser.runtime.onMessage.addListener(function (message, sender, sendResponse: any) {
    ...
    if (message?.from === MESSAGE_ORIGIN_BACKGROUND) {
        console.log('Received message from background: ' + JSON.stringify(message), 'type:', message.type)
...
function collectTwitterBookmarks(mode: string) {
    // TODO: 这个会把消息通道给干掉
    console.log('fn collectTwitterBookmarks init', window.location.pathname, window.top === window, mode)
