...
export default class LogseqClient {
  ...
  private getPage = async (
    pageIdentity: LogseqPageIdentity,
  ): Promise<LogseqPageIdentity> => {
    const resp: LogseqPageIdentity = await this.baseJson(
  ...
  private searchLogseqInternal = async (
    ...
    const result: LogseqSearchResult = {
      pages: await Promise.all(
        pages.map(
          async (page) =>
            await this.getPage({ name: page } as LogseqPageIdentity),
        ),
      ),
